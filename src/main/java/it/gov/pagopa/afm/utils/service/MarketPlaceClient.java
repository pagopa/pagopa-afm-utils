...
public interface MarketPlaceClient {
  ...
  @Retryable(
      ...
      maxAttemptsExpression = "${retry.marketplace.maxAttempts}",
      backoff = @Backoff(delayExpression = "${retry.marketplace.maxDelay}"))
  @DeleteMapping(value = "/psps/{idPSP}/bundles/{idBundle}", consumes = MediaType.APPLICATION_JSON_VALUE)
  ResponseEntity<Void> removeBundle(
      @Size(max = 35) 
      @Parameter(description = "PSP identifier", required = true) 
      @PathVariable("idPSP") String idPsp,
      @Parameter(description = "Bundle identifier", required = true) 
      @PathVariable("idBundle") String idBundle);
}
