...
public class CDIService {
    ...
    public List<CDI> getWorkableCDIs() {
    	return cdisRepository.getWorkableCDIs();
    }
    ...
    public List<BundleResponse> turnCDIToBundles(List<CDI> cdis){
    	log.info("CDIService - turnCDIToBundles executed at: " + LocalDateTime.now() + " for CDI list with size: " + cdis.size());
		...
		for (CDI cdi: cdis) {
			...
			if (null != cdi.getCdiStatus() && !cdi.getCdiStatus().equals(StatusType.FAILED)) {
				...
				try {
					...
					log.info(String.format("CDI status updated [idCdi=%s, status=%s, errorDesc=%s]", 
	...
	public void setCDIToProcessingStatus(List<CDI> items) {
		for (CDI cdi: items) {
			...
			CDI updated = this.updateCDI(cdi);
			log.info(String.format("CDI status updated [idCdi=%s, status=%s]", 
					Optional.ofNullable(updated).map(result -> updated.getIdCdi()), 
					Optional.ofNullable(updated).map(result -> updated.getCdiStatus())));
		}
	}
